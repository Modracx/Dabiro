...
async function doLogin(req, cfg, httpRes) {
    ...
    const hasPendingSsh = !!(req.session && req.session.pendingSsh && tunnelStatus(req.session.pendingSsh).up);

    if ((cfg.ssh && cfg.ssh.enabled) || hasPendingSsh) {
        if (hasPendingSsh) {
            ssh = req.session.pendingSsh;
            host = '127.0.0.1';
            port = ssh.localPort || ssh.port;
        } else {
