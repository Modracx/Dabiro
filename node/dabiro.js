...
function renderLogin(V) {
    ...
    let hasPendingSsh = false;
    let pendingSshInfo = null;
    if (V.req && V.req.session && V.req.session.pendingSsh) {
        const st = tunnelStatus(V.req.session.pendingSsh);
        if (st.up) {
            hasPendingSsh = true;
            pendingSshInfo = Object.assign({}, V.req.session.pendingSsh, { localPort: st.port });
        } else {
            delete V.req.session.pendingSsh;
        }
    }
...
<div class="login-wrap">
  <div class="login-card">
    ...
    <div class="tabs" style="margin-bottom:12px" role="tablist">
      <button type="button" class="tab ${!hasPendingSsh ? 'active ' : ''}hov" data-pane="direct" role="tab">${ico('plug-zap')} Direct</button>
      <button type="button" class="tab ${hasPendingSsh ? 'active ' : ''}hov" data-pane="ssh" role="tab">${ico('shield-check')} SSH Tunnel</button>
    ...
    <div class="pane ${!hasPendingSsh ? 'on' : ''}" id="pane-uri-holder">
    ...
    <!-- SSH Step Indicator -->
    <div id="sshStepper" class="ssh-step-indicator" style="${hasPendingSsh ? '' : 'display:none;'}">
      <div class="step-pill ${hasPendingSsh ? 'done' : 'active'}" id="stepPill1">
        <span class="step-num">1</span>
        <span class="step-lbl">SSH Bastion</span>
      </div>
      <span class="step-sep">${ico('chevron-right')}</span>
      <div class="step-pill ${hasPendingSsh ? 'active' : ''}" id="stepPill2">
    ...
    <div id="sshStep1" style="display:none;">
      ...
      <div class="subpanel" style="margin-bottom:12px">
        <div class="t">${ico('shield-check')} SSH Bastion Details</div>
        <div class="row">
          <div class="field" style="flex:3"><label for="sshHost">SSH host</label>
            <input type="text" name="ssh_host" id="sshHost" class="input-sm" placeholder="bastion.example.com" value="${h((pendingSshInfo && pendingSshInfo.host) || '')}"></div>
          <div class="field" style="flex:1;min-width:80px"><label for="sshPort">Port</label>
            <input type="number" name="ssh_port" id="sshPort" class="input-sm" value="${h((pendingSshInfo && pendingSshInfo.port) || '22')}"></div>
        </div>
        <div class="field"><label for="sshUser">SSH username</label>
          <input type="text" name="ssh_user" id="sshUser" class="input-sm" placeholder="ubuntu" value="${h((pendingSshInfo && pendingSshInfo.user) || '')}"></div>
      ...
      <button type="button" id="sshConnectBtn" class="btn btn-primary btn-block hov" style="margin-top:6px;padding:9px">
        <span class="btn-label">Connect SSH Tunnel</span>${ico('arrow-right')}
      </button>
    </div>
    ...
    <form method="post" id="loginForm" autocomplete="off">
      ...
      <input type="hidden" name="use_ssh" id="useSsh" value="${hasPendingSsh ? '1' : '0'}">

      <!-- Active SSH Chip (shown on Step 2) -->
      <div id="sshActiveChip" class="ssh-active-chip" style="${hasPendingSsh ? '' : 'display:none;'}">
        <div class="ssh-active-info">
          <span class="dot"></span>
          <div class="grow ellipsis">
            <span style="font-weight:600">SSH Tunnel Active:</span>
            <span id="sshActiveDetails" class="mono">${h(hasPendingSsh ? (((pendingSshInfo.user ? pendingSshInfo.user + '@' : '') + pendingSshInfo.host) + ' (port ' + pendingSshInfo.localPort + ')') : '')}</span>
          </div>
        </div>
        <button type="button" class="btn btn-ghost btn-sm hov" id="sshDisconnectBtn" title="Disconnect SSH Tunnel">
          ${ico('unplug')} Disconnect
        </button>
      </div>
      ...
      <div class="col-db">
        ...
        <div class="row" id="hostRow">
          <div class="field" style="flex:3"><label for="dbHost" id="hostLabel">${h(t(V.lang, 'host_label'))}</label>
            <input type="text" name="db_host" id="dbHost" value="127.0.0.1" required></div>
      ...
      <button type="submit" name="login" value="1" id="dbConnectBtn" class="btn btn-primary btn-block hov" style="margin-top:6px;padding:9px">
        <span class="btn-label">${h(hasPendingSsh ? 'Connect to Database' : t(V.lang, 'connect_button'))}</span>${ico('arrow-right')}
      </button>

      <button type="button" id="sshBackBtn" class="btn btn-ghost btn-sm btn-block hov" style="margin-top:6px;${hasPendingSsh ? '' : 'display:none;'}">
        ${ico('arrow-left')} Edit SSH settings
      </button>
    ...
    const panes = (batches || []).map((b, i) => {
        ...
        return `<div class="rt-pane ${i === 0 ? '' : 'hidden'}" data-rt="${i}">
          <div class="card-head" style="align-items:center;flex-wrap:wrap;gap:8px">
            <h3 class="mono small ellipsis" style="max-width:50%">${h(String(b.sql).slice(0, 120))}</h3>
            <span class="right flex" style="align-items:center;gap:10px">
              <span class="muted small">${b.ms} ms${b.rows !== null ? ` &middot; ${b.rows.length} rows` : ''}
                ${b.truncated ? ' &middot; <span class="badge badge-warn">first 1000 shown</span>' : ''}</span>
              ${exportDropdown}
            </span></div>
          ${inner}</div>`;
    }).join('');
  ...
  <div class="card"><form method="post" id="sqlForm">
    ...
    <div class="sql-editor">
      ...
      <div class="sql-bar">
        ...
        <div class="dropdown">
          <button type="button" class="btn btn-default btn-sm hov" data-dropdown>
            ${ico('download')} <span>Export Result</span> ${ico('chevron-down', '', 11)}
          </button>
          <div class="dropdown-menu">
            <button type="submit" name="export_result_format" value="csv" class="dropdown-item">
              ${ico('file-spreadsheet')} <span>Export as CSV</span> <span class="badge">.csv</span>
            </button>
            <button type="submit" name="export_result_format" value="json" class="dropdown-item">
              ${ico('braces')} <span>Export as JSON</span> <span class="badge">.json</span>
            </button>
            <button type="submit" name="export_result_format" value="sql" class="dropdown-item">
              ${ico('database')} <span>Export as SQL</span> <span class="badge">.sql</span>
            </button>
            <button type="submit" name="export_result_format" value="xml" class="dropdown-item">
              ${ico('file-code-2')} <span>Export as XML</span> <span class="badge">.xml</span>
            </button>
            <div class="dropdown-divider"></div>
            <button type="submit" name="export_query" value="1" class="dropdown-item">
              ${ico('file-text')} <span>Save query script</span> <span class="badge">.sql</span>
            </button>
          </div>
        </div>
...
async function exportQueryResult(res, db, rawSql, format) {
    format = ['csv', 'json', 'sql', 'xml'].includes(String(format).toLowerCase()) ? String(format).toLowerCase() : 'csv';
    const stamp = new Date().toISOString().replace(/[:T]/g, '-').slice(0, 19);
    const basename = 'query_result_' + stamp;
    ...
    const stmts = splitSql(rawSql);
    if (!stmts.length) {
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        return res.status(400).send('No SQL statement provided.');
    }
    ...
    try {
        for (const s of stmts) {
            lastSql = s;
            const r = await db.run(s);
            if (r.isSelect) rows = r.rows || [];
        }
    } catch (e) {
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        return res.status(400).send('Error executing query for export: ' + e.message);
    }
    ...
    if (format === 'sql') {
        ...
        await write('-- Dabiro query result export\n');
        await write(`-- Engine:   ${db.getType()} ${await db.serverVersion()}\n`);
        await write(`-- Database: ${db.getDatabase()}\n`);
        await write(`-- Date:     ${new Date().toISOString()}\n`);
        await write(`-- Query:    ${String(lastSql).replace(/[\r\n]+/g, ' ')}\n\n`);

        let cols = null;
        for (const r of rows) {
            if (cols === null) cols = Object.keys(r).map((c) => db.quoteId(c)).join(', ');
            const vals = Object.values(r).map(quoteVal).join(', ');
            await write(`INSERT INTO ${db.quoteId('result')} (${cols}) VALUES (${vals});\n`);
        }
        return res.end();
    }
    ...
    if (rows.length) {
        await write(Object.keys(rows[0]).map(cell).join(',') + '\n');
        for (const r of rows) {
            await write(Object.values(r).map(cell).join(',') + '\n');
        }
    }
...
async function doLogin(req, cfg, httpRes) {
    ...
    const hasPendingSsh = !!(req.session && req.session.pendingSsh);

    if (hasPendingSsh) {
        ssh = req.session.pendingSsh;
        const tr = await ensureTunnel(ssh);
        if (!tr.ok) return tr.error;
        host = '127.0.0.1';
        port = tr.port;
        ssh.localPort = tr.port;
        req.session.pendingSsh = ssh;
    } else if (cfg.ssh && cfg.ssh.enabled) {
    ...
    if (res !== true) {
        if (ssh && !hasPendingSsh) closeTunnel(ssh);
        return res;
    }
...
app.all('*', upload.single('sql_file'), async (req, res) => {
    ...
    const V = {
        ...
        error: null, success: null, connError: null, req,
    };
    ...
    try {
        ...
        if (body.logout !== undefined && validateCsrf(req, body.csrf_token)) {
            ...
            if (req.session && req.session.pendingSsh) closeTunnel(req.session.pendingSsh);
        ...
        if (action === 'ssh_connect') {
            ...
            if (!SSH2) {
                return jsonOut({ ok: false, error: 'The ssh2 package is not installed. Run: npm install ssh2' }, 400);
            }

            const sshHost = String(body.ssh_host || '').trim();
            const sshPort = parseInt(body.ssh_port, 10) || 22;
            const sshUser = String(body.ssh_user || '').trim();
            const sshAuth = body.ssh_auth || 'agent';
            const targetHost = String(body.target_host || '127.0.0.1').trim() || '127.0.0.1';
            const targetPort = parseInt(body.target_port, 10) || 3306;
            const localPort = parseInt(body.ssh_local_port, 10) || 0;

            if (!sshHost) return jsonOut({ ok: false, error: 'SSH host is required.' }, 400);
            if (!sshUser && sshAuth !== 'agent') return jsonOut({ ok: false, error: 'SSH username is required.' }, 400);
            ...
            if (req.session && req.session.pendingSsh) {
                closeTunnel(req.session.pendingSsh);
                delete req.session.pendingSsh;
            }

            const tr = await ensureTunnel(sshCfg);
            if (!tr.ok) return jsonOut({ ok: false, error: tr.error || 'Could not establish SSH tunnel.' }, 400);

            sshCfg.localPort = tr.port;
            req.session.pendingSsh = sshCfg;
        ...
        if (action === 'ssh_status') {
            if (req.session && req.session.pendingSsh) {
                const st = tunnelStatus(req.session.pendingSsh);
                if (st.up) {
                    return jsonOut({
                        ok: true,
                        connected: true,
                        port: st.port,
                        host: req.session.pendingSsh.host || '',
                        user: req.session.pendingSsh.user || '',
                        target_host: req.session.pendingSsh.targetHost || '127.0.0.1',
                        target_port: req.session.pendingSsh.targetPort || 3306,
                    });
                }
            }
            return jsonOut({ ok: true, connected: false });
        }
        ...
        if (AJAX_ACTIONS.includes(action)) {
            ...
            if (action === 'cell_update') {
                ...
                const postDb = String(body.db || query.db || '');
                const postSchema = String(body.schema || query.schema || '');
                if (postDb || postSchema) await focusConnection(req, db, postDb || V.selectedDb, postSchema || V.selectedSchema);
                const id = await rowIdentity(db, tb, keys);
                if (!id.hasPk && !Object.keys(keys).length) return jsonOut({ ok: false, error: 'This table has no primary key, so rows cannot be edited inline.' }, 400);
        ...
        // ── SQL console ──
        let batches = null;
        if (body.execute_sql !== undefined || body.export_query !== undefined || body.export_result_format !== undefined || body.action === 'export_query_result') {
            if (!validateCsrf(req, body.csrf_token)) {
                error = 'Security token validation failed.';
            } else {
                const raw = String(body.sql || body.query_sql || '').trim();
                ...
                if (body.export_result_format !== undefined || body.action === 'export_query_result') {
                    const fmt = String(body.export_result_format || body.export_format || 'csv');
                    await exportQueryResult(res, db, raw, fmt);
                    if (db) { await db.close(); db = null; }
                    return;
                }
                if (raw) {
